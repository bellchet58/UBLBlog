...
import com.ublblog.utils.JSONUtils;
...
public class BackArticleController extends BaseController {
        ...
        try {  
            ...
            if (!article.getTitle().isEmpty() && !article.getContent().isEmpty()
                ...
                if (articleService.addArticle(article)) {
                	...
                	logger.debug("the articleDto  is {}",articleDto.getTitle());
    ...
    public String updateArticles(@RequestBody String bodyString) {
        ...
        try {  
        	...
        	Article article = (Article) JSONUtils.String2Object(
        			json.get("article").toString(), Article.class);
        	logger.debug("the article is id: {},title: {},content: {},createTime: {},deleted: {}",article.getId(),article.getTitle(),article.getContent(),article.getCreateTime(),article.getDeleted());
            //参数正常
            if (article.getId() != null) {
                //如果更新成功,返回SUCCESS
                if (articleService.updateArticle(article)) { 
                	logger.debug("update article itself success~");
                	//如果执行的不是删除操作
                	RelArticleTag rel = new RelArticleTag();
                	rel.setArticleId(article.getId());
                	//先清除之前的文章标签关系
                	articleTagService.deleteArticleTag(rel);
                	logger.debug("clear the tag relationship successfully~");
                	...
                	if (article.getDeleted() != Common.IS_DELETE) {
                		...
                		if (tags != null && tags.size() > 0 ) {             		
                			//然后插入最新的文章标签
                			logger.debug("updating the tag relationship");
                			articleTagService.saveRelArticleTag(article, tags);               		
                		}                		
                	}
                	return SUCCESS; 
                ...
                logger.error("ArticleController.updateArticles();", e.getMessage());
        }
        //返回FAIL
        return FAIL;
    }
