...
<body style="background:none">
	...
	<div class="ope-area">
		<!--添加新的文章-->
		<div class="edit-new">
			<form id="formUser" action="${rc.contextPath}/ucenter/update" method="POST" charset="utf-8">
				<div class="title">
				<ul>
						<li>
							<label>显示名称：</label>
						...
						<li>
							<label>头像URL：</label>
							<input type="text" id="avatarUrl" value="<#if user.image?exists>${user.image?string}</#if>" name="image"/>
							<div id="uploadForm">
									<input id="avatarFile" type="file" name="avatar"/>
									<input id="avatarName" type="text" name="name"/>
									<button id="avatarUploadBtn" class="btn btn-primary" type="button">upload</button>
							</form>
						</li>
					</ul>
				</div>
				<div class="edit-con">
					...
					<p>
						...
						<input id="updateUserBtn" type="submit" value="确认发布"/>		
					</p>					
				</div>
